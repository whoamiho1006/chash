...
namespace chash {
	...
	const uint8_t sha384::PADDING[128] = { 0x80, 0, };
	...
	void sha384::update(const uint8_t* inBytes, size_t inSize) {
		if (!_init) {
	...
	void sha384::finalize(digest_t& outDigest) {
		if (!_init) {
			throw invalid_state_error("Can't perform anything for non-initiated algorithm!");
		}
    ...
    void sha384::transform(uint64_t* w)
    {
        uint32_t t;
        uint64_t temp1;
        uint64_t temp2;
