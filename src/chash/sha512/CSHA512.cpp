...
namespace chash {
    ...
    const uint8_t CSHA512::PADDING[128] = { 0x80, 0, };

	CSHA512::CSHA512()
		: IAlgorithm(EAlgorithm::SHA512), _init(false), _count(0)
	{
	}
    ...
    bool CSHA512::finalize(IDigest* outDigest) {
        if (!_init) {
            setError(EAlgorithmErrno::InvalidState);
            return false;
        }

        if (outDigest->size() < 64) {
    ...
    void CSHA512::transform(uint64_t* w)
    {
        uint32_t t;
        uint64_t temp1;
        uint64_t temp2;
